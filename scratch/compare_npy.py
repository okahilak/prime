#!/usr/bin/env python3
"""
Compare two .npy files field by field on their common top-level keys.
Expects .npy files that contain a pickled dict (saved as a 0-d object array).

Usage:
    python compare_npy.py REF.npy NEW.npy [--rtol 1e-6] [--atol 1e-10]
    python compare_npy.py REF.npy NEW.npy --keys bad_channels ica
"""
import argparse
import numpy as np
import pandas as pd
...
def load(path):
    """Load a .npy file containing a pickled dict (0-d object array)."""
    raw = np.load(path, allow_pickle=True)
    # Handle 0-d object array wrapping a dict
    if isinstance(raw, np.ndarray) and raw.dtype == object and raw.shape == ():
        return raw.item()
    # Handle plain dict (shouldn't happen with np.load, but just in case)
    if isinstance(raw, dict):
        return raw
    raise ValueError(
        f"Expected .npy file containing a dict, got {type(raw)} "
        f"(dtype={getattr(raw, 'dtype', None)}, shape={getattr(raw, 'shape', None)})"
    )
...
def main():
    p = argparse.ArgumentParser(
        description="Compare two .npy files (containing pickled dicts) field by field."
    )
